...
interface YearGroup {
    year: number;
    engagements: SerializedHackathon[];
}

interface HackathonGalleryListProps {
...
export default function HackathonGalleryList({ yearGroups, imageManifest }: HackathonGalleryListProps) {
                                        ...
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex items-start gap-3 flex-1 min-w-0">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="text-base font-semibold text-neutral-900 mb-1 flex items-center gap-2 flex-wrap">
                                                        <span>{engagement.title}</span>
                                                        <span className={`text-xs px-2 py-0.5 rounded-full capitalize font-semibold ${roleBadgeColors[engagement.role] || 'bg-neutral-100 text-neutral-600'}`}>
                                                            {engagement.role}
                                                        </span>
                                                        {hasImages && (
                                                    ...
                                                    {engagement.result && (
                                                        <p className="text-xs font-semibold text-green-700">{engagement.result}</p>
                                                    )}
                                                    {engagement.description && (
                                                        <p className="text-xs text-neutral-500">{engagement.description}</p>
                                                    )}
                                                </div>
                                            </div>
