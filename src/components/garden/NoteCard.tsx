...
export const NoteCard: React.FC<NoteCardProps> = ({
    ...
    return (
        <div className="group flex h-full min-h-[280px] items-center">
            <div
                className={`noteCardEffect h-full w-full ${
                    randomRotation >= 0 ? 'hide-after' : 'hide-before'
                }`}
                style={transformStyles}
            >
                {isDraft ? (
                    <div className="relative h-full cursor-not-allowed p-2" style={backgroundColorStyle}>
                        {content}
                    </div>
                ) : (
                    <a
                        className="relative block h-full p-2 transition-all hover:scale-105 hover:shadow-md"
                        href={`${currentPath}/${data.name.replace(/\.md$/, '')}`}
